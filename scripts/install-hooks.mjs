#!/usr/bin/env node
// Install or remove unidle's Claude Code Stop hook. Zero dependencies. Node 16+.
...
const HELP = `usage: install-hooks.mjs [--scope ID] [--shared | --global] [--uninstall]
  ...
  --scope ID    pin the hook to one validated .unidle/ID pipeline
  --uninstall   remove only unidle handlers, preserving sibling handlers`;
...
const backup = target + ".unidle.bak";
...
const command = quote(process.execPath) + " " + quote(script) + " --unidle" + (scope ? " --scope " + scope : "");

const isOurHandler = (handler) => {
  if (!isObject(handler) || typeof handler.command !== "string") return false;
  const value = handler.command;
  return value.includes("stop-hook.mjs") && (value.includes("--unidle") || value.includes(script) || /[\\/]unidle[\\/]/i.test(value));
};
...
if (uninstall) {
  if (!removed) {
    console.log("Nothing to remove: no unidle Stop hook found in " + target);
    process.exit(0);
  }
  persist(nextSettings(keptGroups));
  console.log("Removed unidle Stop hook (" + removed + " handler(s)) from " + target + ". Sibling handlers were preserved.");
  process.exit(0);
}
...
console.log("Installed unidle Stop hook into " + target + "\n" +
  "  command: " + command + "\n" +
  "  scope:   " + (scope ? scope : "resolved per session/pipeline") + "\n" +
  "  backup:  " + (raw ? backup : "not needed for a new file") + "\n" +
  "  note:    keep .unidle/ and .unidle-hook-state.json ignored; they are local coordination state." +
