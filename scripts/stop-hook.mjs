#!/usr/bin/env node
// Claude Code Stop hook for one unidle pipeline. Zero dependencies. Node 16+.

import { existsSync, readFileSync, unlinkSync } from "node:fs";
import { resolve } from "node:path";
import {
  UNIDLE_DIR, gateState, hookStatePath, parseGates, qualify, resolveTarget,
...
if (scopeIndex !== -1 && (!scopeArg || validateScopeId(scopeArg))) {
  allow("unidle: installed hook has an invalid --scope value; not blocking.");
}
...
if (target.ambiguous) {
  allow("unidle: " + target.ambiguous.length + " pipelines under " + UNIDLE_DIR +
    "/ (" + target.ambiguous.join(", ") + ") and none bound to this session; not blocking.");
}
if (target.error && !target.ambiguous) allow("unidle: " + target.error + "; not blocking.");
...
try {
  ...
  allow("unidle: could not update the serialized hook state (" + error.message + "); not blocking to avoid a trap.");
}

const where = target.scope ? " [scope " + target.scope + "]" : "";
const outstanding = [...invalid, ...unmet];
if (sessionState.blocks > MAX_BLOCKS) {
  allow("unidle: releasing after " + MAX_BLOCKS + " blocks without gate progress" + where +
...
console.log(JSON.stringify({
  decision: "block",
  reason: "unidle" + where + ": " + outstanding.length + " gate/ledger item(s) need work: " + list +
