...
export const UNIDLE_DIR = ".unidle";
export const LOCK_DIR = join(UNIDLE_DIR, "locks");
...
export function scopeRoot(root, scope) {
  return join(root, UNIDLE_DIR, scope);
}
...
export function resolveTarget(options = {}) {
  ...
  const wanted = options.scope || process.env.UNIDLE_SCOPE || null;
  if (wanted) {
    ...
    if (!scopes.includes(wanted)) {
      return {
        ...
        error: "no such scope \"" + wanted + "\" under " + UNIDLE_DIR + "/ (have: " +
          (scopes.join(", ") || "none") + ")",
      };
    }
  ...
  if (scopes.length > 1) {
    ...
    return {
      ...
      error: scopes.length + " pipelines present (" + scopes.join(", ") +
        "); pass --scope <id> or set UNIDLE_SCOPE. Refusing to guess.",
    };
  }
...
export function statusLogPath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "status.log") : join(root, "unidle-status.log");
}

export function hookStatePath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "hook-state.json") : join(root, ".unidle-hook-state.json");
}

function assertSafeStatePath(root, target) {
  const stateRoot = join(resolve(root), UNIDLE_DIR);
