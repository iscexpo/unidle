...
run modes:
  ...
  --shell PATH          command shell (UNIDLE_SHELL, then platform default)
...
Approvals live outside the repository under ~/.unidle/approved by default.
...
if (!target.files.length && action !== "--release") {
  failUsage("no gate files found (looked for " + UNIDLE_DIR + "/<scope>/, then GATES.md and gates/*.md under " + root + ")");
}
...
const approvalDir = resolve(process.env.UNIDLE_APPROVAL_DIR || join(homedir(), ".unidle", "approved"));
if (!opt.status && pathIsInside(root, approvalDir)) failUsage("UNIDLE_APPROVAL_DIR must be outside the repository root");
