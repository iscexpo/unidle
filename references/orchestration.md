...
1. **Plan before fan-out.** Create `.unidle/<scope>/PLAN.md`, `.unidle/<scope>/GATES.md`, and one ledger per leaf and branch from the templates. Fix interfaces, naming, toolchain, dependencies, and exact ownership before dispatch.
...
5. **Verify each return independently.** Re-run the returned leaf's runnable gates, including already checked gates:
   ...
   ```text
   node <skill-dir>/scripts/gate-check.mjs --root . --cwd . --reverify .unidle/<scope>/gates/leaf-1.2.1.md
   ```
...
```text
node <skill-dir>/scripts/gate-check.mjs --root . --cwd . --reverify --jobs 4 .unidle/<scope>/gates/leaf-1.1.1.md .unidle/<scope>/gates/leaf-1.1.2.md
```
