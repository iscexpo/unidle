...
- [ ] N0: this ledger states outcomes that can fail
  CHECK: node <skill-dir>/scripts/gate-lint.mjs .unidle/<scope>/gates/node-<id>.md
  EXPECT: LINT OK
  EVIDENCE: pending

- [ ] N1: every named child leaf is reverified from its exact ledger
  CHECK: node <skill-dir>/scripts/gate-check.mjs --root . --cwd . --reverify --jobs 1 .unidle/<scope>/gates/leaf-<a>.md .unidle/<scope>/gates/leaf-<b>.md
  EXPECT: ALL MET
  EVIDENCE: pending
...
N0 lints this ledger; keep it first and name this node's own path.
