...
- [ ] G0: this ledger states outcomes that can fail
  CHECK: node <skill-dir>/scripts/gate-lint.mjs GATES.md
  EXPECT: LINT OK
  EVIDENCE: pending
...
- Keep between five and twelve live gates; G0 lints this ledger and prints
  LINT OK only when its oracles can fail honestly. A scoped leaf names its
  own path under .unidle/<scope>/gates/ in the G0 CHECK instead of GATES.md.
...
- Scoped and legacy discovery anchor checks at the repository root. When this
  ledger is named explicitly from `.unidle/`, pass an explicit repository
