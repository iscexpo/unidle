...
function sandbox() {
  const dir = mkdtempSync(join(tmpdir(), "unidle-stress-"));
  return {
    dir,
...
test("leases: an explicit release cleans orphaned leases after a scope directory is gone", async () => {
  ...
  try {
    ...
    const lockNames = readdirSync(s.path(".unidle/locks"));
    assert(!lockNames.some((name) => name.endsWith(".lease")), "orphan lease remained: " + lockNames.join(", "));

    s.write(".unidle/live/gates/leaf-live.md", "# malformed zero-gate ledger\n");
...
test("hook: repeated 64-writer bursts are serialized without lost increments", async () => {
  ...
  try {
    ...
    for (let round = 1; round <= 3; round++) {
      rmSync(s.path(".unidle-hook-state.json"), { force: true });
      ...
      const state = JSON.parse(s.read(".unidle-hook-state.json"));
...
test("hook: sessions remain isolated and completion/no-gates clears stale state", async () => {
  ...
  try {
    ...
    let state = JSON.parse(s.read(".unidle-hook-state.json"));
    ...
    state = JSON.parse(s.read(".unidle-hook-state.json"));
    ...
    assert(!existsSync(s.path(".unidle-hook-state.json")), "no-gates path did not clear final state");
  } finally { s.cleanup(); }
});
...
test("installer: uninstall preserves a sibling in the same matcher group and writes a backup", async () => {
  ...
  try {
      ...
      hooks: {
        Stop: [{
          ...
          hooks: [
            ...
            { type: "command", command: "node /tmp/unidle/scripts/stop-hook.mjs --unidle" },
          ],
        }],
      },
    }, null, 2) + "\n";
    ...
    assert(s.read(".claude/settings.local.json.unidle.bak") === original, "backup did not preserve original bytes");
  } finally { s.cleanup(); }
});
...
test("installer: a matching command with broken managed fields is repaired", async () => {
  ...
  try {
    ...
    const managed = after.hooks.Stop.flatMap((group) => group.hooks)
      .filter((item) => typeof item.command === "string" && item.command.includes("--unidle"));
...
test("installer: scope input is validated and local state warning is explicit", async () => {
  ...
  try {
    ...
    has(good.out, ".unidle/");
    has(good.out, ".unidle-hook-state.json");
    const settings = JSON.parse(s.read(".claude/settings.local.json"));
