#!/usr/bin/env node
// run-tests.mjs : behavioural tests for the unidle enforcement scripts.
...
const APPROVAL_ROOT = mkdtempSync(join(tmpdir(), "unidle-test-approvals-"));
...
function sandbox() {
  const dir = mkdtempSync(join(tmpdir(), "unidle-test-"));
  return {
    dir,
    write(rel, text) {
...
function run(script, args, opts = {}) {
  return new Promise((res) => {
    ...
    const child = execFile(process.execPath, [script, ...actualArgs], {
      ...
      env: { ...process.env, UNIDLE_APPROVAL_DIR: APPROVAL_ROOT, ...(opts.env || {}) },
    }, (err, stdout, stderr) => {
...
test("scope: running one pipeline leaves the other pipeline alone", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "# Gates: api1\n\n" + gate("G1", "api", echoOk("API-OK"), "API-OK"));
    s.write(".unidle/web/gates/leaf-1.md", "# Gates: web1\n\n" + gate("G1", "web", echoOk("WEB-OK"), "WEB-OK"));
    const r = await run(GATE_CHECK, ["--scope", "api"], { cwd: s.dir });
    assertHas(r.out, "PASS leaf-1:G1");
    assertHas(r.out, "[scope api]");
    assertHas(s.read(".unidle/api/gates/leaf-1.md"), "- [x] G1", "api gates");
    assertHas(s.read(".unidle/web/gates/leaf-1.md"), "- [ ] G1", "web gates must be untouched");
  } finally { s.cleanup(); }
});
...
test("scope: two pipelines and no scope is refused, not guessed", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "# Gates: api\n\n" + gate("G1", "api", echoOk("API-OK"), "API-OK"));
    s.write(".unidle/web/gates/leaf-1.md", "# Gates: web\n\n" + gate("G1", "web", echoOk("WEB-OK"), "WEB-OK"));
    const r = await run(GATE_CHECK, [], { cwd: s.dir });
    assert(r.code === 2, "expected exit 2, got " + r.code);
    assertHas(r.out, "2 pipelines present");
    assertHas(r.out, "api");
    assertHas(r.out, "web");
    assertHas(s.read(".unidle/api/gates/leaf-1.md"), "- [ ] G1", "api must be untouched");
    assertHas(s.read(".unidle/web/gates/leaf-1.md"), "- [ ] G1", "web must be untouched");
  } finally { s.cleanup(); }
});

test("scope: UNIDLE_SCOPE selects the pipeline", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "# Gates: api\n\n" + gate("G1", "api", echoOk("API-OK"), "API-OK"));
    s.write(".unidle/web/gates/leaf-1.md", "# Gates: web\n\n" + gate("G1", "web", echoOk("WEB-OK"), "WEB-OK"));
    const r = await run(GATE_CHECK, [], { cwd: s.dir, env: { UNIDLE_SCOPE: "web" } });
    assertHas(r.out, "[scope web]");
    assertHas(s.read(".unidle/web/gates/leaf-1.md"), "- [x] G1", "web gates");
    assertHas(s.read(".unidle/api/gates/leaf-1.md"), "- [ ] G1", "api must be untouched");
  } finally { s.cleanup(); }
});
...
test("scope: a single pipeline needs no flag", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/solo/GATES.md", "# Gates: solo\n\n" + gate("G1", "solo", echoOk("S-OK"), "S-OK"));
    const r = await run(GATE_CHECK, [], { cwd: s.dir });
...
test("state: unmet gates are reported with file-qualified ids", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "# Gates: 1\n\n" + gate("G1", "a", null, null));
    s.write(".unidle/api/gates/leaf-2.md", "# Gates: 2\n\n" + gate("G1", "b", null, null));
...
test("leases: overlapping OWNS across pipelines is refused", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "OWNS: src/shared/**\n\n# Gates\n\n" + gate("G1", "a", null, null));
    s.write(".unidle/web/gates/leaf-1.md", "OWNS: src/shared/util.ts\n\n# Gates\n\n" + gate("G1", "b", null, null));
...
test("leases: disjoint OWNS both succeed, and release frees them", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "OWNS: src/api/**\n\n# Gates\n\n" + gate("G1", "a", null, null));
    s.write(".unidle/api/gates/leaf-2.md", "OWNS: src/web/**\n\n# Gates\n\n" + gate("G1", "b", null, null));
    const a = await run(GATE_CHECK, ["--scope", "api", "--leaf", "leaf-1", "--claim"], { cwd: s.dir });
...
test("status log: appends, and appends survive concurrency", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-1.md", "# Gates\n\n" + gate("G1", "a", null, null));
    await Promise.all([
    ...
    const log = s.read(".unidle/api/status.log");
...
test("hook: does not block on a pipeline this session does not own", async () => {
  ...
  try {
    ...
    s.write(".unidle/web/gates/leaf-1.md", "# Gates\n\n" + gate("G1", "not started", null, null));
...
test("hook: blocks on its own scope, naming qualified ids", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/api/gates/leaf-7.md", "# Gates\n\n" + gate("G3", "unfinished", null, null));
    s.write(".unidle/web/gates/leaf-1.md", "# Gates\n\n- [x] G1: done\n  EVIDENCE: proven\n");
    const r = await run(STOP_HOOK, ["--scope", "api"], { cwd: s.dir, stdin: JSON.stringify({ cwd: s.dir }) });
...
test("hook: each pipeline keeps its own loop-guard counter", async () => {
  ...
  try {
    ...
    const apiState = JSON.parse(s.read(".unidle/api/hook-state.json"));
...
test("install: uninstall removes our entry and leaves others alone", async () => {
  ...
  try {
    ...
    assertHas(r.out, "Removed unidle Stop hook");
    const cfg = JSON.parse(s.read(".claude/settings.local.json"));
    ...
    s.write(".claude/settings.local.json", JSON.stringify({
      hooks: {
        Stop: [{
          hooks: [{
            type: "command",
            command: 'node "/home/me/.claude/skills/unidle/scripts/stop-hook.mjs"',
          }],
        }],
      },
    }, null, 2));
