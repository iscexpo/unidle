...
function sandbox() {
  ...
  const dir = realpathSync(mkdtempSync(join(tmpdir(), "unidle-hardening-")));
  const approvals = realpathSync(mkdtempSync(join(tmpdir(), "unidle-approval-")));
  return {
    dir, approvals,
...
function gateRun(s, args, options = {}) {
  ...
  return run(GATE_CHECK, actual, {
    cwd: s.dir,
    env: { UNIDLE_APPROVAL_DIR: s.approvals, ...(options.env || {}) },
  });
}
...
test("approval: status is read-only and an unapproved CHECK is printed but not run", async () => {
  ...
  try {
    ...
    const status = await gateRun(s, ["--status"], { approve: false, env: { UNIDLE_SHELL: "definitely-missing-shell" } });
...
test("approval: storage failures use the infrastructure exit code", async () => {
  ...
  try {
    ...
    const result = await gateRun(s, ["--approve"], {
      approve: false,
      env: { UNIDLE_APPROVAL_DIR: join(s.approvals, "records") },
    });
...
test("leases: unsafe declarations, unknown leaves, and wildcard witnesses fail closed", async () => {
  const s = sandbox();
  try {
    s.write(".unidle/a/gates/leaf-a.md", "OWNS: src/a*.js\n\n" + gate("G1", "a", null, null));
    s.write(".unidle/b/gates/leaf-b.md", "OWNS: src/ab*.js\n\n" + gate("G1", "b", null, null));
    ...
    s.write(".unidle/dot-a/gates/leaf.md", "OWNS: src/./same.js\n\n" + gate("G1", "dot a", null, null));
    s.write(".unidle/dot-b/gates/leaf.md", "OWNS: src/same.js\n\n" + gate("G1", "dot b", null, null));
    ...
    s.write(".unidle/prefix-a/gates/leaf.md", "OWNS: src/api\n\n" + gate("G1", "prefix a", null, null));
    s.write(".unidle/prefix-b/gates/leaf.md", "OWNS: src/api/**\n\n" + gate("G1", "prefix b", null, null));
    ...
    s.write(".unidle/c/gates/leaf-c.md", "OWNS: ../outside/**\n\n" + gate("G1", "c", null, null));
